    ...
    for idx, (metric, title) in enumerate(metrics, 1):
        ...
        for S in S_vals:
            for D in D_vals:
                ...
                if b_data:
                    ax.plot(b_data, [S]*len(b_data), times, marker='o', label=f'S={S},D={D}')
            ...
            for S in S_vals:
                ...
                for D in D_vals:
                    if (B, S, D) in data_dict:
                        ...
                        times.append(data_dict[(B, S, D)][metric])
                if d_data:
                    ax.plot([B]*len(d_data), [S]*len(d_data), times, marker='^', alpha=0.7)
...
def benchmark_te_layernorm(
    ...
    for B, S, D in configs:
        ...
        hidden_states = torch.randn(
            S, B, D, device="cuda", dtype=torch.float32
        )
