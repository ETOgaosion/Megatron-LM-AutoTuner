...
def plot_results(results, output_dir):
    ...
    for batch_size, data in sorted(batch_size_groups.items()):
        ...
        ax1.plot(seqlens, hidden_sizes, fwd_times, marker='o', label=f'Batch={batch_size}')
    ...
    for batch_size, data in sorted(batch_size_groups.items()):
        ...
        ax2.plot(seqlens, hidden_sizes, bwd_times, marker='o', label=f'Batch={batch_size}')
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/linear_benchmark_results.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    for batch_size in batch_sizes:
        for seqlen in seqlens:
            for hidden_size in hidden_sizes:
                x = torch.randn(batch_size, seqlen, hidden_size, device=device, dtype=torch.float16)
...
def get_args():
    ...
    parser.add_argument("--num_iters", type=int, default=100, help="Number of benchmark iterations.")
    parser.add_argument(
        "--output_dir",
        type=str,
...
if __name__ == "__main__":
    args = get_args()
    test_linear(num_warmup=args.num_warmup, num_iters=args.num_iters, output_dir=args.output_dir)
