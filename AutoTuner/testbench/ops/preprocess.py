...
class PreprocessForTest(CommonOpsForTest):
    ...
    def _forward(self, input_ids: Tensor, position_ids: Tensor, attention_mask: Tensor, packed_seq_params: PackedSeqParams) -> Tensor:
        ...
        if not self.config.multi_latent_attention:
            ...
            rotary_seq_len = self.rotary_pos_emb.get_rotary_seq_len(
                inference_context = None, transformer = None, transformer_input = decoder_input, transformer_config = self.config, packed_seq_params = packed_seq_params
            )
        ...
        # print("Preprocess output shape:", rotary_pos_emb.shape)
        print("Decoder input shape:", decoder_input.shape)
        print("Attention mask shape:", attention_mask.shape)
        
        return rotary_pos_emb, preproc_output, attention_mask, packed_seq_params
    
    def __call__(self, *args, **kwargs):
        return self.forward(*args, **kwargs)
    
    def forward(
       self, input_ids: Tensor, position_ids: Tensor, attention_mask: Tensor, packed_seq_params: PackedSeqParams
    ) -> Tensor:
        self.activation_hook.clear()
        with torch.autograd.graph.saved_tensors_hooks(
            self.activation_hook.save_hook, self.activation_hook.load_hook
        ):
            ret = self._forward(input_ids, position_ids, attention_mask, packed_seq_params)
        return ret