...
def plot_and_export_embed_info(config_base_dir, output_dir):
    ...
    for model in MODELS:
        ...
        embedding_weights, transformer_layer_weights = get_model_embed_info(model, config_base_dir)
    ...
    bars2 = axes[0].bar(x, total_transformer_layer_weights, width, 
                    bottom=total_embedding_weights, label='Transformer Layer Weights')
    ...
    # Create stacked bar chart
    ratio = [x / (x + y) for x, y in zip(total_embedding_weights, total_transformer_layer_weights)]
